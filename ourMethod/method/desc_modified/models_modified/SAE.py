...
class SAE(object):
    ...
    """
    def __init__(self, dims, act='relu', drop_rate=0.2, batch_size=32,random_seed=201809,actincenter="tanh",init="glorot_uniform",use_earlyStop=True,save_dir='result_tmp',labels=None): #act relu ### def __init__(self, dims, act='relu', drop_rate=0.2, batch_size=32,random_seed=201809,actincenter="tanh",init="glorot_uniform",use_earlyStop=True,save_dir='result_tmp'): #act relu
        self.dims = dims
        ...
        self.init=init
        self.batch_size = batch_size; self.labels = labels ### self.batch_size = batch_size
        #set random seed
        ...
        self.autoencoders, self.encoder, self.dnn= self.make_autoencoders() ### self.autoencoders ,self.encoder= self.make_autoencoders()
    ...
    def make_autoencoders(self):
        ...
        # output
        y = Dense(self.dims[0], kernel_initializer=self.choose_init(init=self.init,seed=self.random_seed+2*self.n_stacks),name='decoder_0',activation=lambda x: x)(y) # self.actincenter
        z = Dense(18, kernel_initializer=self.choose_init(init=self.init, seed=self.random_seed+18),  activation='softmax')(h) ###
        return Model(inputs=x, outputs=y,name="AE"), Model(inputs=x,outputs=h,name="encoder"), Model(inputs=x,outputs=z,name="dnn") ### return Model(inputs=x, outputs=y,name="AE"),Model(inputs=x,outputs=h,name="encoder")
    ...
    def make_stack(self, ith,random_seed=0):
        ...
        if ith == 0:
            output_act = lambda x:x ### self.actincenter# tanh, or linear
    ...
    def pretrain_stacks(self, x, epochs=20,decaying_step=3):
        """ 
        ...
        """
        features = copy.deepcopy(x) ### features = x
        for i in range(self.n_stacks):
            print( 'Pretraining the %dth layer...' % (i))
            ...
            print ('The %dth layer has been pretrained.' % (i))
            ...
            self.featuresBeforeAE = features = feature_model.predict(features); feature_model.save(os.path.join(self.save_dir,'the%dthStackModel.h5'%(i)))

    def pretrain_autoencoders(self, x, epochs=50):
        """
        ...
        for j in range(math.ceil(epochs/10)):
            lr = pow(10, -j-1)
            print ('learning rate =', lr)
            self.autoencoders.compile(optimizer=SGD(lr, momentum=0.9), loss='mse')
            self.autoencoders.fit(x=x, y=x, batch_size=self.batch_size, epochs=5)

    def fit(self, x, y, epochs=50,decaying_step=3): # use stacked autoencoder pretrain and fine tuning ### def fit(self, x, epochs=300,decaying_step=3): # use stacked autoencoder pretrain and fine tuning
        self.pretrain_stacks(x, epochs=int(epochs/3),decaying_step=decaying_step)
        self.pretrain_autoencoders(x, epochs=epochs)
        # self.pretrain_dnn(x, y, epochs=epochs) ###
    ...
    def extract_feature(self, x):
        """ Extract features from the middle layer of autoencoders(representation). Arguments: x: data """
        return self.encoder.predict(x)
    def pretrain_dnn(self, x, y, epochs=50):
        print('Contruct a new dnn model to make class mission, pretrain this model now.'); simple_adam = keras.optimizers.Adam()
        callbacks=[EarlyStopping(monitor='loss',min_delta=1e-4,patience=10,verbose=1,mode='auto',restore_best_weights = True)]
