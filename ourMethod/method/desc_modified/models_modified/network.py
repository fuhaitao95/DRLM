...
import scanpy as sc # import scanpy.api as sc
import pandas as pd
...
class DescModel(object):
    def __init__(self,
		 ...
		 save_encoder_weights=False,
                 save_encoder_step=5,
                 save_dir="output/result_tmp",
                 kernel_clustering="t", label_init=None, label_ohe = None ### kernel_clustering="t",
        ...
        self.dims = dims; self.delta_label_list=[]; self.label_init=label_init; self.label_ohe = label_ohe ### self.dims = dims
        ...
        #set random seed
        random.seed(random_seed); self.denovo = 0
    ...
    def pretrain(self,n_clusters=None):
        ...
        if self.use_ae_weights:
            if not os.path.isfile(self.save_dir+"/ae_weights.h5"):
                if self.is_stacked:
                    sae.fit(self.x,self.label_ohe,epochs=self.pretrain_epochs); self.featuresBeforeAE=sae.featuresBeforeAE ### sae.fit(self.x,epochs=self.pretrain_epochs)
                else:
                    sae.fit2(self.x,epochs=self.pretrain_epochs)
                self.autoencoder=sae.autoencoders; self.encoder=sae.encoder; self.dnn=sae.dnn; self.denovo=1 #self.compile(optimizer=SGD(0.01,0.9), loss='kld'); self.fit(maxiter=300) # self.hist_1=sae.hist ### self.encoder=sae.encoder
            else:
                featuresBeforeAE0 = load_model(os.path.join(self.save_dir,'the%dthStackModel.h5'%(0)),compile=False).predict(self.x); self.featuresBeforeAE = load_model(os.path.join(self.save_dir,'the%dthStackModel.h5'%(1)),compile=False).predict(featuresBeforeAE0);
                sae.autoencoders.load_weights(os.path.join(self.save_dir,"ae_weights.h5")); sae.encoder.load_weights(os.path.join(self.save_dir,"encoder_weights.h5")); self.featuresBeforeCluster=sae.encoder.predict(self.x); self.autoencoder=sae.autoencoders
                self.encoder=sae.encoder; sae.dnn.load_weights(os.path.join(self.save_dir,"dnn_weights.h5")); self.dnn=sae.dnn; self.featuresBeforeDNN=load_model(os.path.join(self.save_dir,'encoderBeforeDNN.h5')).predict(self.x); self.Embedded_z=load_model(os.path.join(self.save_dir,'encoderAfterdsnModel.h5')).predict(self.x)
                return
        else:
            if self.is_stacked:
                sae.fit(self.x,self.label_ohe,epochs=self.pretrain_epochs); self.featuresBeforeAE=sae.featuresBeforeAE ### sae.fit(self.x,epochs=self.pretrain_epochs)
            ...
            self.dnn=sae.dnn; # self.hist_1=sae.hist
        ...
        if not os.path.isfile(os.path.join(self.save_dir,"ae_weights.h5")):
            ...
            self.encoder.save_weights(os.path.join(self.save_dir,'encoder_weights.h5')); self.dnn.save_weights(os.path.join(self.save_dir,'dnn_weights.h5')) ### self.encoder.save_weights(os.path.join(self.save_dir,'encoder_weights.h5'))
        ...
        self.featuresBeforeCluster=self.encoder.predict(self.x) ### features=np.asarray(features)
        if isinstance(n_clusters,int) and self.label_init == None: ### if isinstance(n_clusters,int):
            ...
            self.n_clusters=n_clusters; cluster_centers=kmeans.cluster_centers_; self.init_centroid=[cluster_centers] ### self.n_clusters=n_clusters
        elif isinstance(n_clusters,int) and self.label_init != None: ### cluster_centers=kmeans.cluster_centers_
            self.init_pred,self.n_clusters,self.init_centroid = getCentroid(self.label_init,n_clusters,features) ### self.init_centroid=[cluster_centers]
        else:
    ...
    def dsn_model(self):
        fc = Dense(18, kernel_initializer=keras.initializers.glorot_uniform(seed=self.random_seed+19), activation='softmax')(self.encoder.output)
        self.dsn = Model(inputs=self.encoder.input, outputs=fc)
    ...
    def fit_on_batch(self,maxiter=1e4,update_interval=200,save_encoder_step=4):
        ...
        #save encoder model
        self.encoder.save(os.path.join(self.save_dir,"encoderBeforeDNN.h5"))
        #load model
        ...
        q=self.model.predict(self.x,verbose=0)
        return Embedded_z,q
    def train_dsn(self, x, y, epochs=30):
        print('pretrain dnn again.'); simple_adam = keras.optimizers.Adam(); callbacks=[EarlyStopping(monitor='acc',min_delta=1e-3,patience=10,verbose=1,mode='auto',restore_best_weights = True)]; self.dsn.compile(loss='categorical_crossentropy', optimizer=simple_adam, metrics=['accuracy'])
        self.hist_3 = self.dsn.fit(x, y, batch_size=self.batch_size, callbacks=callbacks, epochs=epochs, shuffle=True, verbose=1)
    def fit_on_all(self, maxiter=1e3, epochs_fit=5,save_encoder_step=5): # unsupervised
        save_dir=self.save_dir
        #step1 initial weights by louvain,or Kmeans
        self.model.get_layer(name='clustering').set_weights(self.init_centroid)
        # Step 2: deep clustering
        y_pred_last = np.copy(self.init_pred); self.hist = {'loss':[]} # y_pred_last = np.copy(self.init_pred)
        for ite in range(int(maxiter)):
            ...
            if self.use_earlyStop:
                ...
                self.hist_2 = self.model.fit(x=self.x,y=p,epochs=epochs_fit,batch_size=self.batch_size,callbacks=callbacks,shuffle=True,verbose=True) ### self.model.fit(x=self.x,y=p,epochs=epochs_fit,batch_size=self.batch_size,callbacks=callbacks,shuffle=True,verbose=True)
            else:
                self.hist_2 = self.model.fit(x=self.x,y=p,epochs=epochs_fit,batch_size=self.batch_size,shuffle=True,verbose=True) ### self.model.fit(x=self.x,y=p,epochs=epochs_fit,batch_size=self.batch_size,shuffle=True,verbose=True)
        #save encoder model
        self.encoder.save(os.path.join(self.save_dir,"encoderBeforeDNN.h5"))
        #load model
        #encoder=load_model("encoder.h5")
        self.featuresBeforeDNN = np.asarray(self.extract_features(self.x)) # 保存中间参数 # features =
        self.dsn_model(); self.train_dsn(self.x, self.label_ohe, epochs=self.pretrain_epochs); self.encoder.save(os.path.join(self.save_dir,'encoderAfterdsnModel.h5')) # 训练分类 判别类别
        y0=pd.Series(y_pred,dtype='category')
        y0.cat.categories=range(0,len(y0.cat.categories))
        print("The final prediction cluster is:")
        y0Label=y0.value_counts()
        print(y0Label.sort_index(ascending=True))
        self.Embedded_z=Embedded_z=self.extract_features(self.x)
        return Embedded_z,q
...
def getCentroid(label_init,n_clusters,features):
    dataTemp = pd.DataFrame(features)
    dataTemp['label'] = label_init
    cent = np.asarray(dataTemp.groupby('label').mean())
    init_centroid = [cent]
    return label_init,n_clusters,init_centroid